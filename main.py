"""
main.py
Entry point for the Sales Prediction project.
...
def main():
    try:
        ...
        viz = Visualizer()
        viz.plot_sales_vs_spend(df)
        viz.plot_correlation_heatmap(df)
        viz.plot_sales_by_segment_platform(df)
        ...
        for name, model in trained_models.items():
            ...
            prediction_dict[name] = result["y_pred"]
        ...
        print_section("STEP 5: ADVERTISING IMPACT & FORECAST COMPARISON")
        forecast_df = trainer.build_forecast_comparison(prediction_dict, y_test)
        viz.plot_prediction_comparison(forecast_df)

        linear_model = trained_models.get("Linear Regression")
        impact = trainer.advertising_impact_analysis(
            linear_model, preprocessor.preprocessor, feature_names
        )
        if impact:
            print("Advertising impact (Linear Regression coefficients):")
            for feature, coef in sorted(impact.items(), key=lambda x: -abs(x[1])):
                print(f"  {feature}: {coef:.4f}")

        print_section("STEP 6: SAVE BEST MODEL")
