"""
evaluation.py
Regression model evaluation utilities: MAE, RMSE, R^2 for the Sales
Prediction project.
"""
...
class ModelEvaluator:
    ...
    def evaluate(self, model, model_name: str, X_test, y_test) -> dict:
        """Evaluate a single trained regression model on the test set."""
        ...
        logger.info("[%s] MAE=%.3f, RMSE=%.3f, R^2=%.4f", model_name, mae, rmse, r2)
    ...
    def compare_models(self, results: list) -> pd.DataFrame:
        ...
        comparison = pd.DataFrame(
            [
                {"Model": r["model_name"], "MAE": r["mae"], "RMSE": r["rmse"], "R2": r["r2"]}
                for r in results
            ]
